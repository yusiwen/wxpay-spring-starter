...
public class SignatureProvider {
    ...
    public SignatureProvider(WechatMetaContainer wechatMetaContainer) {
        ...
        List<HttpMessageConverter<?>> messageConverters = restOps.getMessageConverters();
        messageConverters.removeIf(
                httpMessageConverter -> httpMessageConverter instanceof MappingJackson2XmlHttpMessageConverter);
        restOps.setMessageConverters(messageConverters);
    ...
    public String requestSign(String tenantId, String method, String canonicalUrl, String body) {
        ...
        String nonceStr = nonceStrGenerator.generateId().toString().replaceAll("-", "");
        ...
        // 生成token
        String token = String.format(TOKEN_PATTERN, wechatMetaBean.getV3().getMchId(), nonceStr, timestamp, serialNo,
                encode);
    ...
    public boolean responseSignVerify(ResponseSignVerifyParams params) {
        ...
        final String signatureStr = createSign(true, params.getWechatpayTimestamp(), params.getWechatpayNonce(),
                params.getBody());
        Signature signer = Signature.getInstance("SHA256withRSA", BC_PROVIDER);
            ...
            certificates.forEach(objectNode -> {
                ...
                try {
                    ...
                    throw new PayException(
                            "An error occurred while generating the wechat v3 certificate, reason : " + e.getMessage());
                }
            });
        }
    }
    ...
    public X509WechatCertificateInfo getCertificate(String tenantId) {
        for (Map.Entry<String, X509WechatCertificateInfo> entry : CERTIFICATE_MAP.entrySet()) {
