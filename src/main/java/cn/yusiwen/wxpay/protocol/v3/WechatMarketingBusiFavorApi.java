...
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiCouponCodeUploadParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorAssociateInfo;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorBudgetParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorCallbackSettingParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorCreateParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorDeactivateParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorRefundParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorUpdateParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.BusiFavorUseParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.UserBusiCouponQueryParams;
import cn.yusiwen.wxpay.protocol.v3.model.busifavor.UserBusiFavorQueryParams;
...
public class WechatMarketingBusiFavorApi extends AbstractApi {
     ...
     * 创建商家券券批次API
     * <p>
     * 商家券介绍详见 <a target= "_blank" href=
     * "https://pay.weixin.qq.com/wiki/doc/apiv3/wxpay/marketing/busifavor/chapter1_1.shtml">微信支付商家券</a>
     *
     * @param params
     *            the params
     ...
     * 上传预存code API
     * <p>
     * 商家券的Code码可由微信后台随机分配，同时支持商户自定义。 如商家已有自己的优惠券系统，可直接使用自定义模式。
    ...
    public WechatResponseEntity<ObjectNode> uploadCouponCodes(BusiCouponCodeUploadParams params) {
        ...
        this.client().withType(WechatPayV3Type.MARKETING_BUSI_FAVOR_UPLOAD_COUPON_CODES, params)
                .function((type, queryParams) -> {
                    URI uri = UriComponentsBuilder.fromHttpUrl(type.uri(WeChatServer.CHINA)).build()
                            .expand(queryParams.getStockId()).toUri();
     ...
     * 设置商家券事件通知地址API
     * <p>
     * 用于设置接收商家券相关事件通知的URL，可接收商家券相关的事件通知、包括发放通知等。 需要设置接收通知的URL，并在商户平台开通 <a target= "_blank" href=
     * "https://pay.weixin.qq.com/index.php/xphp/cmkt_product/index#/pages/product/product?pid=3">营销事件推送</a>
     * 的能力，即可接收到相关通知。
     * <p>
     * 营销事件推送：<a target= "_blank" href=
     * "https://pay.weixin.qq.com/index.php/core/home/login?return_url=%2Findex.php%2Fxphp%2Fcmkt_product%2Findex#/pages/product/product?pid=3">点击开通产品权限</a>。由商家券批次创建方登录Pay平台，操作开通
     * <p>
     * 注意：
     * <ul>
    ...
    public WechatResponseEntity<ObjectNode> associate(BusiFavorAssociateInfo associateInfo) {
        ...
        this.client().withType(WechatPayV3Type.MARKETING_BUSI_FAVOR_ASSOCIATE, associateInfo)
                .function(this::associateFunction).consumer(wechatResponseEntity::convert).request();
        return wechatResponseEntity;
    }
    ...
    public WechatResponseEntity<ObjectNode> budget(BusiFavorBudgetParams params) {
        ...
        this.client().withType(WechatPayV3Type.MARKETING_BUSI_FAVOR_BUDGET, params).function((type, budgetParams) -> {
            URI uri = UriComponentsBuilder.fromHttpUrl(type.uri(WeChatServer.CHINA)).build()
                    .expand(budgetParams.getStockId()).toUri();
    ...
    public WechatResponseEntity<ObjectNode> updateStock(BusiFavorUpdateParams params) {
        ...
        this.client().withType(WechatPayV3Type.MARKETING_BUSI_FAVOR_UPDATE, params).function((type, updateParams) -> {
            URI uri = UriComponentsBuilder.fromHttpUrl(type.uri(WeChatServer.CHINA)).build()
                    .expand(updateParams.getStockId()).toUri();
