...
public abstract class AbstractApi {
    ...
    protected RequestEntity<?> post(URI uri, Object params) {
        try {
            return RequestEntity.post(uri).header("Pay-TenantId", tenantId).body(mapper.writeValueAsString(params));
    ...
    protected RequestEntity<?> post(URI uri, Object params, HttpHeaders httpHeaders) {
        try {
            return RequestEntity.post(uri).header("Pay-TenantId", tenantId).headers(httpHeaders)
    ...
    protected RequestEntity<?> get(URI uri, HttpHeaders httpHeaders) {
        return RequestEntity.get(uri).header("Pay-TenantId", tenantId).headers(httpHeaders).build();
    }
    ...
    protected RequestEntity<?> patch(URI uri, Object params) {
        try {
            return RequestEntity.patch(uri).header("Pay-TenantId", tenantId).body(mapper.writeValueAsString(params));
        } catch (JsonProcessingException e) {
            throw new PayException("wechat app pay json failed");
        }
    }
     ...
     *      AbstractApi#billResource(String)AbstractApi#billResource(String)AbstractApi#billResource(String)
     */
    ...
    public ResponseEntity<Resource> downloadTradeBill(TradeBillParams tradeBillParams) {
        ...
        this.client().withType(WechatPayV3Type.TRADE_BILL, tradeBillParams).function((wechatPayV3Type, params) -> {
            ...
            URI uri = UriComponentsBuilder.fromHttpUrl(wechatPayV3Type.uri(WeChatServer.CHINA)).queryParams(queryParams)
                    .build().toUri();
            return get(uri);
        }).consumer(wechatResponseEntity::convert).request();
        String downloadUrl = Objects.requireNonNull(wechatResponseEntity.getBody()).get("download_url").asText();
     ...
     *
     * @param downloadUrl
     *            格式为 {@code https://api.mch.weixin.qq.com/v3/billdownload/file?token=xxx}
     * @param filename
     *            文件名称包含扩展名
     * @return the response entity
     */
