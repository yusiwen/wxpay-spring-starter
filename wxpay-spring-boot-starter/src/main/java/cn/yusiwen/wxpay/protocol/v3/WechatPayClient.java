...
public class WechatPayClient {
    ...
    public static class Executor<M> {
         ...
         *            the rest operations
         */
        Executor(WechatPayV3Type wechatPayV3Type, M model, SignatureProvider signatureProvider,
        ...
        public void request() {
            ...
            WechatRequestEntity<?> wechatRequestEntity = WechatRequestEntity.of(requestEntity,
        ...
        private <T> void doExecute(WechatRequestEntity<T> requestEntity) {
            ...
            if (!statusCode.is2xxSuccessful()) {
                PayException payException = new PayException("wechat pay server error, Request-ID " + requestId
                        + " , statusCode " + statusCode + ",result : " + body);
            ...
            if (signatureProvider.responseSignVerify(params)) {
                ...
                PayException payException = new PayException(
                        "wechat pay signature verify failed, Request-ID " + requestId);
                payException.setResponse(responseEntity);
                throw payException;
            }
        }
        ...
        private <T> String doDownload(WechatRequestEntity<T> requestEntity) {
            ...
            if (!statusCode.is2xxSuccessful()) {
                PayException payException = new PayException("wechat pay server error, Request-ID  " + requestId
                        + " , statusCode " + statusCode + ",result : " + responseEntity);
                payException.setResponse(responseEntity);
                throw payException;
            }
