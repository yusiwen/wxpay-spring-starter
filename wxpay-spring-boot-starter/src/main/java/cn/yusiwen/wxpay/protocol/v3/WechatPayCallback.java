...
public class WechatPayCallback {
     ...
     */
    @SneakyThrows
    public Map<String, String> couponCallback(ResponseSignVerifyParams params,
            Consumer<CouponConsumeData> consumeDataConsumer) {
     ...
     */
    @SneakyThrows
    public Map<String, String> transactionCallback(ResponseSignVerifyParams params,
            Consumer<TransactionConsumeData> consumeDataConsumer) {
        String data = this.callback(params, EventType.TRANSACTION);
     ...
     */
    @SneakyThrows
    public Map<String, String> combineTransactionCallback(ResponseSignVerifyParams params,
            Consumer<CombineTransactionConsumeData> consumeDataConsumer) {
        String data = this.callback(params, EventType.TRANSACTION);
     ...
     * <p>
     * 该链接是通过商户
     * <a target= "_blank" href= "https://pay.weixin.qq.com/wiki/doc/apiv3/wxpay/payscore/chapter3_1.shtml">创建支付分订单</a>
     * 提交notify_url参数，必须为https协议。如果链接无法访问，商户将无法接收到微信通知。 通知url必须为直接可访问的url，不能携带参数。示例：
     * “https://pay.weixin.qq.com/wxpay/pay.action”
     *
     * @param params
     *            the params
     * @param payScoreConsumer
     *            the pay score consumer
     * @return the map
     * @since 1.0.0.RELEASE
     */
    @SneakyThrows
    public Map<String, String> payscoreUserOrderCallback(ResponseSignVerifyParams params,
        ...
        if (Objects.equals(eventType, EventType.PAYSCORE_USER_CONFIRM.event)) {
            ...
            PayScoreUserConfirmConsumeData confirmConsumeData = MAPPER.readValue(data,
                    PayScoreUserConfirmConsumeData.class);
     ...
     * <p>
     * 普通授权模式是通过[商户入驻配置申请表]提交service_notify_url设置，预授权模式是通过[商户预授权]提交的notify_url设置，必须为https协议。如果链接无法访问，商户将无法接收到微信通知。
     ...
     */
    @SneakyThrows
    public Map<String, String> permissionCallback(ResponseSignVerifyParams params,
            Consumer<PayScoreUserPermissionConsumeData> consumeDataConsumer) {
        CallbackParams callbackParams = resolve(params);
        String eventType = callbackParams.getEventType();
        boolean closed;
     ...
     * <p>
     * 该链接是通过商户<a target= "_blank" href=
     * "https://pay.weixin.qq.com/wiki/doc/apiv3/wxpay/discount-card/chapter3_1.shtml">预受理领卡请求</a>中提交notify_url参数，必须为https协议。如果链接无法访问，商户将无法接收到微信通知。
     * 通知url必须为直接可访问的url，不能携带参数。示例： “https://pay.weixin.qq.com/wxpay/pay.action”
     *
     * @param params
     *            the params
     * @param discountCardConsumer
     *            the discount card consumer
     * @return the map
     */
    @SneakyThrows
    public Map<String, ?> discountCardCallback(ResponseSignVerifyParams params,
        ...
        if (Objects.equals(eventType, EventType.DISCOUNT_CARD_AGREEMENT_ENDED.event)) {
            String data = this.decrypt(callbackParams);
            DiscountCardAgreementEndConsumeData agreementEndConsumeData = MAPPER.readValue(data,
                    DiscountCardAgreementEndConsumeData.class);
            ...
            String data = this.decrypt(callbackParams);
            DiscountCardAcceptedConsumeData acceptedConsumeData = MAPPER.readValue(data,
                    DiscountCardAcceptedConsumeData.class);
            ...
            String data = this.decrypt(callbackParams);
            DiscountCardUserPaidConsumeData paidConsumeData = MAPPER.readValue(data,
                    DiscountCardUserPaidConsumeData.class);
     ...
     * <p>
     * 该链接是通过商户<a target= "_blank" href=
     * "https://pay.weixin.qq.com/wiki/doc/apiv3/wxpay/marketing/busifavor/chapter3_7.shtml">设置商家券事件通知地址API</a>中提交notify_url参数，必须为https协议。如果链接无法访问，商户将无法接收到微信通知。
     * 通知url必须为直接可访问的url，不能携带参数。示例： “https://pay.weixin.qq.com/wxpay/pay.action”
     *
     * @param params
     *            the params
     ...
     */
    @SneakyThrows
    public Map<String, String> busiFavorReceiveCallback(ResponseSignVerifyParams params,
            Consumer<BusiFavorReceiveConsumeData> consumeDataConsumer) {
        CallbackParams callbackParams = resolve(params);
        String eventType = callbackParams.getEventType();
     ...
     */
    @SneakyThrows
    public Map<String, String> refundCallback(ResponseSignVerifyParams params,
            Consumer<RefundConsumeData> consumeDataConsumer) {
        CallbackParams callbackParams = resolve(params);
        String eventType = callbackParams.getEventType();
